...
import '../../application/controllers/calculator_notifier.dart';
...
class ButtonGrid extends ConsumerWidget {
  ...
  Widget _buildButtonRow(BuildContext context, List<_ButtonConfig> configs) {
    return Expanded(
      child: Row(
        children:
            configs.map((config) => _buildButton(context, config)).toList(),
      ),
    );
  }
  ...
  Widget _buildButton(BuildContext context, _ButtonConfig config) {
    return CalcButton(
      ...
      backgroundColor: config.backgroundColor ?? AppColors.buttonSurface,
